...
*/5 * * * pgrep -x "/usr/local/bin/cloudflared" > /dev/null || nohup /usr/local/bin/cloudflared tunnel --protocol http2 --loglevel fatal run --token $CLOUDFLARED_TOKEN > /dev/null 2>&1 &
...
nohup /usr/local/bin/cloudflared tunnel --protocol http2 --loglevel fatal run --token "$CLOUDFLARED_TOKEN" > /dev/null 2>&1 &
